<xsl:stylesheet
  ...
  <xsl:template name="processBillPackageInfo">
    ...
    <BillPackage>
      ...
      <xsl:if test="($changeBillPackage) > 0 and $bse/newApplicationId/text() and not($bse/applicationId/text() = $bse/newApplicationId/text())">
        <ChangedFamilyID>
          <xsl:value-of select="$bse/newApplicationId"/>
        </ChangedFamilyID>
      </xsl:if>
      <!--<xsl:if test="string-length($newBillPackage) = 0"> WR-125145 -->
      <xsl:if test="$carrierBillingInfo/billingGroupId">
        <IVLCustGroupID>
          <xsl:value-of select="$carrierBillingInfo/billingGroupId"/>
        </IVLCustGroupID>
      </xsl:if>
      <xsl:if test="$billInformation/carrierBillingInformation/billingGroupId">
        <IVLCustGroupID>
      ...
      <!--</xsl:if>-->
      <!--<xsl:if test="string-length($newBillPackage) = 0">  WR-125145 -->
      <xsl:if test="$carrierBillingInfo/billingUnitId">
        <IVLBillPackageNo>
          <xsl:value-of select="$carrierBillingInfo/billingUnitId"/>
        </IVLBillPackageNo>
      </xsl:if>
      <xsl:if test="$billInformation/carrierBillingInformation/billingUnitId">
        <IVLBillPackageNo>
      ...
      <xsl:if test="$bse/transaction/memberTransactionTypes/text() = 'NEW_ENROLLMENT' or $bse/transaction/memberTransactionTypes/text() = 'CARRIER_BILLING_INFORMATION' or
            $bse/transaction/memberTransactionTypes/text() = 'ADD_OR_UPDATE_PAYMENT_METHOD' or $bse/transaction/memberTransactionTypes/text() = 'ADD_SUBSCRIBER'">
      ...
      <xsl:if test="string-length($newBillPackage) > 0 or string-length($changeBillPackage) > 0">
        <BillRecordType>
          <xsl:choose>
            <xsl:when test="string-length($newBillPackage) > 0">BPN</xsl:when>
      ...
      <xsl:if test="string-length($newBillPackage) > 0 or string-length($changeBillPackage) > 0">
        <xsl:if test="$carrierBillingInfo/responsibleParty/personName/firstName/text()">
          <xsl:if test="$bse/transaction/memberTransactionTypes/text() != 'BENEFIT_ORIGINAL_EFFECTIVE_DATE'">
            <CustomerFirstName>
        ...
        <xsl:if test="($memberBenefitInfo/coverageDates and string-length($newBillPackage) > 0) or $bse/transaction/memberTransactionTypes/text() = 'BENEFIT_ORIGINAL_EFFECTIVE_DATE'">
          <RenewalDay>
            <xsl:choose>
        ...
        <xsl:if test="($bse/transaction/memberTransactionTypes/text() = 'ADD_OR_UPDATE_PAYMENT_METHOD' or string-length($newBillPackage) > 0) and ($memberBenefitInfo/initialPayment/Payment or $memberBenefitInfo/recurringPayment/Payment or $billInformation/billPaymentMethod)">
          <BillPaymentMethod>
            <xsl:choose>
              ...
              <xsl:when test="$memberBenefitInfo/initialPayment/Payment/ACH or $billInformation/billPaymentMethod = 'NON_RECURRING_EFT'">
                <xsl:value-of select="'NFT'"/>
              </xsl:when>
              <xsl:when test="$billInformation/billPaymentMethod = 'CHECK'">
      ...
      <MemberInfo>
        <xsl:if test="string-length($newSubscriber) > 0 or string-length($changeSubscriber) > 0 or string-length($generalDependentChange) > 0">
          ...
          <xsl:if test="not(normalize-space($hideSubscriberRelationCd) = 'true')">
            ...
            <HIPAACntrctTierCode>
              <xsl:choose>
                <xsl:when test="$benefit/coverageLevelName/text() = 'EmployeeOnly'">SUB</xsl:when>
                <xsl:when test="$benefit/coverageLevelName/text() = 'EmployeeAndSpouse'">SUBS</xsl:when>
                <xsl:when test="$benefit/coverageLevelName/text() = 'Family'">SUBF</xsl:when>
                <xsl:when test="$benefit/coverageLevelName/text() = 'EmployeeAndChildren'">SUBC</xsl:when>
                <xsl:when test="$benefit/coverageLevelName/text() = 'EmployeeAndOneDependent'">SUBD1</xsl:when>
                <xsl:when test="$benefit/coverageLevelName/text() = 'EmployeeAndTwoDependents'">SUBD2</xsl:when>
                <xsl:when test="$benefit/coverageLevelName/text() = 'EmployeeAndSpouseOrDomesticPartner'">SUBSPD</xsl:when>
                <xsl:when test="$benefit/coverageLevelName/text() = 'EmployeeAndSpouseOrCivilUnionParter'">SUBSCU</xsl:when>
              </xsl:choose>
            </HIPAACntrctTierCode>
    ...
    <xsl:if test="string-length($newDependent) > 0 or string-length($changeDependent) > 0">
      <DependentInfo>
        ...
        <xsl:if test="string-length($newSubscriber) > 0 or
                ...
                $transactionType/text() = 'CHANGE_DEPENDENT_FAMILY_CODE'">
          <DependentFamilyCode>
            <xsl:choose>
