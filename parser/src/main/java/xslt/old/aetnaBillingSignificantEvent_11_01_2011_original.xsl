<xsl:stylesheet
  ...
  <xsl:template match="/">
    <eDirectFile>
      <xsl:attribute name="xsi:noNamespaceSchemaLocation">eDirect-EBS Trigger V0.9.xsd</xsl:attribute>
      <eDirectRecord>
  ...
  <xsl:template name="processHeader">
    <HeaderInfo>
      ...
      <HeaderTransactionID>
        <xsl:value-of select="/BillingSignificantEvents/HeaderInfo/HeaderTransactionID"/>
      </HeaderTransactionID>
      <HeaderTransactionType>EBSBATINTF</HeaderTransactionType>
      <HeaderDateTime>
        <xsl:value-of select="/BillingSignificantEvents/HeaderInfo/HeaderDateTime"/>
      </HeaderDateTime>
  ...
  <xsl:template name="containsNonRiderTransactions">
    ...
    <xsl:choose>
      ...
      <!--<xsl:when test="$bse/memberBenefit[productType != 'RIDER' and @referenceId = $bse/transaction/transactionObjectReferenceId]">
        true
      </xsl:when>-->
      <xsl:otherwise>
        true
  ...
  <xsl:template name="processBillPackageInfo">
    ...
    <BillPackage>
      ...
      <FamilyID>
        <xsl:value-of select="$bse/applicationId"/>
      </FamilyID>

      <xsl:if test="(($changeBillPackage) > 0 and $bse/newApplicationId/text() and not($bse/applicationId/text() = $bse/newApplicationId/text())) or not(string-length($changeBillPackage) > 0 and $bse/transaction/memberTransactionTypes/text() = 'BENEFIT_ORIGINAL_EFFECTIVE_DATE')">
        <ChangedFamilyID>
          <xsl:value-of select="$bse/newApplicationId"/>
        </ChangedFamilyID>
      </xsl:if>
      <xsl:if test="$carrierBillingInfo/billingGroupId and not(string-length($changeBillPackage) > 0 and $bse/transaction/memberTransactionTypes/text() = 'BENEFIT_ORIGINAL_EFFECTIVE_DATE')">
        <IVLCustGroupID>
          <xsl:value-of select="$carrierBillingInfo/billingGroupId"/>
        </IVLCustGroupID>
      </xsl:if>
      <xsl:if test="$billInformation/carrierBillingInformation/billingGroupId and not(string-length($changeBillPackage) > 0 and $bse/transaction/memberTransactionTypes/text() = 'BENEFIT_ORIGINAL_EFFECTIVE_DATE')">
        <IVLCustGroupID>
          <xsl:value-of select="$billInformation/carrierBillingInformation/billingGroupId"/>
        </IVLCustGroupID>
      </xsl:if>
      <xsl:if test="$carrierBillingInfo/billingUnitId and not(string-length($changeBillPackage) > 0 and $bse/transaction/memberTransactionTypes/text() = 'BENEFIT_ORIGINAL_EFFECTIVE_DATE')">
        <IVLBillPackageNo>
          <xsl:value-of select="$carrierBillingInfo/billingUnitId"/>
        </IVLBillPackageNo>
      </xsl:if>
      <xsl:if test="$billInformation/carrierBillingInformation/billingUnitId and not(string-length($changeBillPackage) > 0 and $bse/transaction/memberTransactionTypes/text() = 'BENEFIT_ORIGINAL_EFFECTIVE_DATE')">
        <IVLBillPackageNo>
          <xsl:value-of select="$billInformation/carrierBillingInformation/billingUnitId"/>
        </IVLBillPackageNo>
      </xsl:if>
      ...
      <xsl:if test="string-length($newBillPackage) > 0 or string-length($changeBillPackage) > 0">
        ...
        <xsl:if test="string-length($newBillPackage) > 0 or string-length($changeBillPackage) > 0">
          ...
          <xsl:if test="$carrierBillingInfo/responsibleParty/address/state/text()">
            <CountrySubCode>
              <xsl:value-of select="$carrierBillingInfo/responsibleParty/address/state"/>
            </CountrySubCode>
          </xsl:if>
          <xsl:if test="$carrierBillingInfo/responsibleParty/address/country/text()">
            <USAddressCountryCode>
              <xsl:value-of select="$carrierBillingInfo/responsibleParty/address/country"/>
            </USAddressCountryCode>
          </xsl:if>
        ...
        <xsl:if test="$memberBenefitInfo/coverageDates and string-length($newBillPackage) > 0">
          <RenewalDay>
            <xsl:choose>
        ...
        <xsl:if test="($bse/transaction/memberTransactionTypes/text() = 'ADD_OR_UPDATE_PAYMENT_METHOD' or string-length($newBillPackage) > 0) and ($memberBenefitInfo/initialPayment/Payment or $memberBenefitInfo/recurringPayment/Payment or $billInformation/billPaymentMethod)">
          <BillPaymentMethod>
            <xsl:choose>
              ...
              <xsl:when test="($memberBenefitInfo/initialPayment/Payment/ACH or $billInformation/billPaymentMethod = 'NON_RECURRING_EFT') and not($memberBenefitInfo/initialPayment/Payment/Check and ($memberBenefitInfo/initialPayment/Payment/Check/NameOnCheck/text() or $memberBenefitInfo/initialPayment/Payment/Check/CheckNumber/text()))">
                <xsl:value-of select="'NFT'"/>
              </xsl:when>
              <xsl:when test="($memberBenefitInfo/initialPayment/Payment/Check and ($memberBenefitInfo/initialPayment/Payment/Check/NameOnCheck/text() or $memberBenefitInfo/initialPayment/Payment/Check/CheckNumber/text()))or $billInformation/billPaymentMethod = 'CHECK'">
                <xsl:value-of select="'CHK'"/>
              </xsl:when>
            </xsl:choose>
          </BillPaymentMethod>
        </xsl:if>
        <xsl:if test="string-length($newBillPackage) > 0 ">
          <xsl:choose>
  ...
  <xsl:template name="isChangeBillPackage">
    ...
    <!--todo-->
    <xsl:choose>
      <xsl:when test="$bse/transaction/memberTransactionTypes/text() = 'UPDATE_SUBSCRIBER'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction/memberTransactionTypes/text() = 'CARRIER_BILLING_INFORMATION'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction/memberTransactionTypes/text() = 'BILLING_TYPE_CHANGE'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction/memberTransactionTypes/text() = 'BENEFIT_ORIGINAL_EFFECTIVE_DATE'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction/memberTransactionTypes/text() = 'UPDATE_APPLICATION_ID'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction/billingTransactionTypes/text()">
        true
      </xsl:when>
    </xsl:choose>
  </xsl:template>
        ...
        <xsl:if test="string-length($newSubscriber) > 0 or string-length($changeSubscriber) > 0 or string-length($generalDependentChange) > 0">
          ...
          <xsl:if test="not(normalize-space($hideSubscriberPlanNameValue) = 'true')">
            <SubscriberPlanName>
              ...
              <xsl:value-of select="substring($benefit/planName,1,50)"/>
            </SubscriberPlanName>
          </xsl:if>
          <xsl:if test="not(normalize-space($hideSubscriberRelationCd) = 'true')">
            <SubscriberZipCode>
            ...
            <HIPAACntrctTierCode>
              <xsl:choose>
                ...
                <xsl:when test="$benefit/coverageLevelName/text() = 'EmployeeAndSpouseOrCivilUnionParter' or $benefit/coverageLevelName/text() = 'Subscriber + Spouse/Civil Union Partner'">SUBSCU</xsl:when>
              </xsl:choose>
            </HIPAACntrctTierCode>
          ...
          <xsl:if test="(string-length($newSubscriber) > 0 or string-length($changeSubscriber) > 0) and
                $benefit/carrierDefinedFields/carrierDefinedField[type/text() = 'FINAL_RATEUP_FACTOR']/value and
            ...
            <SubscriberFinalRateUpFactor>
              <xsl:value-of select="$benefit/carrierDefinedFields/carrierDefinedField[type/text() = 'FINAL_RATEUP_FACTOR']/value"/>
            </SubscriberFinalRateUpFactor>
          </xsl:if>
          <xsl:if test="(string-length($newSubscriber) > 0 or string-length($changeSubscriber) > 0) and
                $benefit/carrierDefinedFields/carrierDefinedField[type/text() = 'INDIVIDUAL_RATEUP_FACTOR']/value and
            ...
            <SubscriberIndRateUpFactor>
              <xsl:value-of select="$benefit/carrierDefinedFields/carrierDefinedField[type/text() = 'INDIVIDUAL_RATEUP_FACTOR']/value"/>
            </SubscriberIndRateUpFactor>
          </xsl:if>
          <xsl:if test="string-length($newSubscriber) > 0">
          ...
          <xsl:if test="$benefit/carrierDefinedFields/carrierDefinedField[type/text() = 'CANCEL_REASON']/value">
            <TermReasonCode>
              <xsl:value-of select="$benefit/carrierDefinedFields/carrierDefinedField[type/text() = 'CANCEL_REASON']/value"/>
            </TermReasonCode>
          </xsl:if>
        </xsl:if>
  ...
  <xsl:template name="isNewSubscriber">
    ...
    <!--todo-->
    <xsl:choose>
      <xsl:when test="$bse/transaction[transactionObjectReferenceId = $subscriberMemberReferenceId]/memberTransactionTypes/text() = 'ADD_SUBSCRIBER'">
        true
      </xsl:when>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template name="isChangeSubscriber">
    ...
    <!--todo-->
    <xsl:choose>
      <xsl:when test="$bse/transaction[transactionObjectReferenceId = $subscriberMemberReferenceId]/memberTransactionTypes/text() = 'UPDATE_SUBSCRIBER'">
        true
      </xsl:when>
      <xsl:when test="$benefit/subscriber/@referenceId and $bse/transaction[transactionObjectReferenceId = $benefit/subscriber/@referenceId]/memberTransactionTypes/text() = 'CANCEL_SUBSCRIBER'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction[transactionObjectReferenceId = $benefit/@referenceId]/memberTransactionTypes/text() = 'INDIVIDUAL_RATEUP_FACTOR_EMPLOYEE'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction[transactionObjectReferenceId = $benefit/@referenceId]/memberTransactionTypes/text() = 'FINAL_RATEUP_FACTOR_EMPLOYEE'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction[transactionObjectReferenceId = $benefit/@referenceId]/memberTransactionTypes/text() = 'PLAN_RATE_EFFECTIVE_DATE'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction[transactionObjectReferenceId = $benefit/@referenceId]/memberTransactionTypes/text() = 'PLAN_RATE_GUARANTEE_EXPIRATION_DATE'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction[transactionObjectReferenceId = $benefit/@referenceId]/memberTransactionTypes/text() = 'UPDATE_BENEFIT_PLAN'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction[transactionObjectReferenceId = $benefit/@referenceId and string-length(childObjectReferenceIds/text()) = 0]/memberTransactionTypes/text() = 'BENEFIT_ORIGINAL_EFFECTIVE_DATE'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction[transactionObjectReferenceId = $benefit/@referenceId and string-length(childObjectReferenceIds/text()) = 0]/memberTransactionTypes/text() = 'BENEFIT_EFFECTIVE_DATE'">
        true
      </xsl:when>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template name="isGeneralChangeDependent">
    ...
    <xsl:param name="benefit"/>
    <xsl:choose>
      <xsl:when test="$bse/transaction[transactionObjectReferenceId = $benefit/@referenceId]/memberTransactionTypes/text() = 'FINAL_RATEUP_FACTOR_DEPENDENT'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction[transactionObjectReferenceId = $benefit/@referenceId]/memberTransactionTypes/text() = 'INDIVIDUAL_RATEUP_FACTOR_DEPENDENT'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction[transactionObjectReferenceId = $benefit/familyMember/memberIdentificationReferenceId]/memberTransactionTypes/text() = 'ADD_DEPENDENT'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction[transactionObjectReferenceId = $benefit/familyMember/memberIdentificationReferenceId]/memberTransactionTypes/text() = 'ADD_ADOPTEE'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction[transactionObjectReferenceId = $benefit/familyMember/memberIdentificationReferenceId]/memberTransactionTypes/text() = 'ADD_NEW_BORN'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction/memberTransactionTypes/text() = 'CHANGE_DEPENDENT_FAMILY_CODE'">
        true
      </xsl:when>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template name="processAnniversaryDate">
    ...
    <xsl:variable name="year" select="number(substring(/BillingSignificantEvents/HeaderInfo/HeaderDateTime, 1, 4)) + 1"/>
    <AnniversaryDate>
  ...
  <xsl:template name="processDependent">
    ...
    <xsl:if test="string-length($newDependent) > 0 or string-length($changeDependent) > 0">
      <DependentInfo>
        ...
        <xsl:if test="$depMemberPersonalId/gender and string-length($newDependent) > 0">
          <DependentSexCode>
            <xsl:choose>
        ...
        <xsl:if test="$depBenefit/carrierDefinedFields/carrierDefinedField[type/text() = 'INDIVIDUAL_RATEUP_FACTOR']/value and (string-length($newDependent) > 0 or $bse/transaction[childObjectReferenceIds/childObjectReferenceId = $dependentMemberReferenceId]/memberTransactionTypes/text() = 'INDIVIDUAL_RATEUP_FACTOR_DEPENDENT')">
          <DependentRateupFactor>
            <xsl:value-of select="$depBenefit/carrierDefinedFields/carrierDefinedField[type/text() = 'INDIVIDUAL_RATEUP_FACTOR']/value"/>
          </DependentRateupFactor>
        </xsl:if>
        <xsl:if test="$depBenefit/carrierDefinedFields/carrierDefinedField[type/text() = 'FINAL_RATEUP_FACTOR']/value and (string-length($newDependent) > 0 or $bse/transaction[childObjectReferenceIds/childObjectReferenceId = $dependentMemberReferenceId]/memberTransactionTypes/text() = 'FINAL_RATEUP_FACTOR_DEPENDENT')">
          <DependentFinalRateUpFactor>
            <xsl:value-of select="$depBenefit/carrierDefinedFields/carrierDefinedField[type/text() = 'FINAL_RATEUP_FACTOR']/value"/>
          </DependentFinalRateUpFactor>
        </xsl:if>
        ...
        <xsl:if test="string-length($newSubscriber) > 0 or
                ...
                $bse/transaction/memberTransactionTypes/text() = 'CHANGE_DEPENDENT_FAMILY_CODE'">
          <DependentFamilyCode>
            <xsl:choose>
        ...
        <xsl:if test="$depBenefit/carrierDefinedFields/carrierDefinedField[type/text() = 'HIPAA_IND']/value and string-length($newDependent) > 0">
  ...
  <xsl:template name="isNewDependent">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$bse/transaction[transactionObjectReferenceId = $dependentMemberReferenceId]/memberTransactionTypes/text() = 'ADD_DEPENDENT'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction[transactionObjectReferenceId = $dependentMemberReferenceId]/memberTransactionTypes/text() = 'ADD_ADOPTEE'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction[transactionObjectReferenceId = $dependentMemberReferenceId]/memberTransactionTypes/text() = 'ADD_NEW_BORN'">
        true
      </xsl:when>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template name="isChangeDependent">
    ...
    <!--todo-->
    <xsl:choose>
      <xsl:when test="$bse/transaction[transactionObjectReferenceId = $dependentMemberReferenceId]/memberTransactionTypes/text() = 'UPDATE_DEPENDENT'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction[childObjectReferenceIds/childObjectReferenceId = $dependentMemberReferenceId]/memberTransactionTypes/text() = 'FINAL_RATEUP_FACTOR_DEPENDENT'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction[childObjectReferenceIds/childObjectReferenceId = $dependentMemberReferenceId]/memberTransactionTypes/text() = 'INDIVIDUAL_RATEUP_FACTOR_DEPENDENT'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction[childObjectReferenceIds/childObjectReferenceId = $dependentMemberReferenceId]/memberTransactionTypes/text() = 'CHANGE_DEPENDENT_FAMILY_CODE'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction[transactionObjectReferenceId = $benefit/familyMember/memberIdentificationReferenceId]/memberTransactionTypes/text() = 'BENEFIT_EFFECTIVE_DATE'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction/memberTransactionTypes/text() = 'CHANGE_DEPENDENT_FAMILY_CODE'">
        true
  ...
  <xsl:template name="hideSubscriberIndividualID">
    <xsl:param name="bse"/>
    <xsl:choose>
      <xsl:when test="$bse/transaction/memberTransactionTypes/text() = 'ADD_DEPENDENT'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction/memberTransactionTypes/text() = 'ADD_ADOPTEE'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction/memberTransactionTypes/text() = 'ADD_NEW_BORN'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction/memberTransactionTypes/text() = 'UPDATE_BENEFIT_PLAN'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction/memberTransactionTypes/text() = 'BENEFIT_EFFECTIVE_DATE'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction/memberTransactionTypes/text() = 'FINAL_RATEUP_FACTOR_DEPENDENT'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction/memberTransactionTypes/text() = 'INDIVIDUAL_RATEUP_FACTOR_DEPENDENT'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction/memberTransactionTypes/text() = 'CHANGE_DEPENDENT_FAMILY_CODE'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction/memberTransactionTypes/text() = 'BILLING_CYCLE_DATE_CHANGE'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction/memberTransactionTypes/text() = 'ADD_OR_UPDATE_PAYMENT_METHOD'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction/memberTransactionTypes/text() = 'BILLING_CYCLE_DATE_CHANGE'">
        true
  ...
  <xsl:template name="hideSubscriberRelationCd">
    <xsl:param name="bse"/>
    <xsl:choose>
      <xsl:when test="$bse/transaction[transactionObjectReferenceId = $bse/memberbenefit/familyMember/memberIdentificationReferenceId]/memberTransactionTypes/text() = 'ADD_DEPENDENT'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction[transactionObjectReferenceId = $bse/memberbenefit/familyMember/memberIdentificationReferenceId]/memberTransactionTypes/text() = 'ADD_ADOPTEE'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction[transactionObjectReferenceId = $bse/memberbenefit/familyMember/memberIdentificationReferenceId]/memberTransactionTypes/text() = 'ADD_NEW_BORN'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction/memberTransactionTypes/text() = 'UPDATE_BENEFIT_PLAN'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction/memberTransactionTypes/text() = 'PLAN_RATE_GUARANTEE_EXPIRATION_DATE'">
        true
      </xsl:when>
      <xsl:when test="$bse/transaction/memberTransactionTypes/text() = 'BENEFIT_EFFECTIVE_DATE'">
        true
      ...
      <xsl:when test="$bse/transaction/memberTransactionTypes/text() = 'CANCEL_SUBSCRIBER'">
        true
      </xsl:when>
    </xsl:choose>
  </xsl:template>
