<xsl:stylesheet
  ...
  <xsl:template name="processBillPackageInfo">
    ...
    <BillPackage>
      ...
      <xsl:if test="(($changeBillPackage) > 0 and $bse/newApplicationId/text() and not($bse/applicationId/text() = $bse/newApplicationId/text())) or
                     not(string-length($changeBillPackage) > 0 and $bse/transaction/memberTransactionTypes/text() = 'BENEFIT_ORIGINAL_EFFECTIVE_DATE')">
        <ChangedFamilyID>
  ...
  <xsl:template name="processDependent">
    ...
    <xsl:if test="(string-length($newDependent) > 0 or string-length($changeDependent) > 0)">
      <DependentInfo>
        ...
        <xsl:if test="$depBenefit/carrierDefinedFields/carrierDefinedField[type/text() = 'INDIVIDUAL_RATEUP_FACTOR']/value and
        ...
        <xsl:if test="$depBenefit/carrierDefinedFields/carrierDefinedField[type/text() = 'FINAL_RATEUP_FACTOR']/value and
